...
db.ref('userorder/').get('value').then((snapshot) => {
    ...
    currentUser.textContent = userValue;
     const userwidth1 = document.getElementById('useractivewidth');
    const userpercent1 = document.getElementById('useractivepercent');
    userwidth1.style.width = `${userValue}%`
    userpercent1.textContent = user+'%'
